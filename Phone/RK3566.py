...
class RK3566(BasePhone):
    ...
    def clear_app(self):
        ...
        self.wait_element("text", "屏幕截图")
        for i in range(30):
            sleep(1)
            if "全部清除" in self.driver.page_source:
                break
            else:
                self.swipe_point(100, 600, 600, 600)
    ...
    def install_app(self, times, apk_name, app_name, file_app="文件极客"):
        ...
        for i in range(10):
            if file_app in self.driver.page_source:
                ...
                sleep(1)
                self.swipe_point(400, 1000, 400, 300)
        ...
        for i in range(times):
            for j in range(10):
                sleep(1)
                if "内部存储空间" in self.driver.page_source:
                    break
                else:
                    self.swipe_point(400, 1000, 400, 300)
            self.wait_and_click_element("text", "内部存储空间")
