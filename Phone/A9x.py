...
class A9x(BasePhone):
    ...
    def clear_app(self):
        ...
        self.wait_and_click_element("text", "清除")
        self.wait_element("text", self.desktop_key)
    ...
    def install_app(self, times, apk_name, app_name, file_app="文件管理"):
        ...
        self.wait_and_click_element("text", "安装包")
        for i in range(times):
            # 通过文件管理(须放桌面)
            self.wait_and_click_element("text", "未安装")
            ...
            while True:
                if app_name in self.driver.page_source:
                    ...
                    sleep(1)
                    self.swipe_point(900, 1200, 100, 1200)
            ...
            # 等待卸载完成
            while True:
                if app_name not in self.driver.page_source:
                    break
